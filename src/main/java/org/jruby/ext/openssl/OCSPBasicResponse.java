...
public class OCSPBasicResponse extends RubyObject {
    ...
    private static final Logger LOG = Logger.getLogger(OCSPBasicResponse.class);
    ...
    private static final String OCSP_NOEXPLICIT = "NOEXPLICIT";
    ...
    private byte[] nonce;
    private List<OCSPSingleResponse> singleResponses = new ArrayList<>();
    ...
    public IRubyObject verify(final ThreadContext context, IRubyObject[] args) {
        ...
        try {
            final BasicOCSPResp basicOCSPResp = newBasicOCSPResp();

            java.security.cert.Certificate signer = findSignerCert(context, asn1BCBasicOCSPResp, convertRubyCerts(certificates), flags);
            if ( signer == null ) return runtime.getFalse();
            if ( (flags & getOCSPConstant(runtime, OCSP_NOINTERN)) == 0 &&
                    (flags & getOCSPConstant(runtime, OCSP_TRUSTOTHER)) != 0 ) {
                flags |= getOCSPConstant(runtime, OCSP_NOVERIFY);
            }
            if ( (flags & getOCSPConstant(runtime, OCSP_NOSIGS)) == 0 ) {
                PublicKey sPKey = signer.getPublicKey();
                if (sPKey == null) return runtime.getFalse();
                try {
                    JcaContentVerifierProviderBuilder jcaVerifierBuilder = newJcaContentVerifierProviderBuilder();
                    boolean valid = basicOCSPResp.isSignatureValid(jcaVerifierBuilder.build(sPKey));
                    if (!valid) return runtime.getFalse();
                }
                catch (Exception e) { // OCSPException|OperatorCreationException
                    LOG.debug(context.runtime, "signature check failed", e);
                    throw newOCSPError(runtime, e);
                }
            }

            if ((flags & getOCSPConstant(runtime, OCSP_NOVERIFY)) == 0) {
                List<X509Cert> untrustedCerts;
                if ((flags & getOCSPConstant(runtime, OCSP_NOCHAIN)) != 0) {
                ...
                try {
                    ctx = X509StoreContext.newStoreContext(context,
                            (X509Store) store,
                            X509Cert.wrap(runtime, signer),
                            RubyArray.newArray(runtime, untrustedCerts)
                    );
                }
                catch (CertificateEncodingException e) {
                    throw newOCSPError(runtime, e);
                ...
                if (!ctx.verify(context).isTrue()) return runtime.getFalse();

                if ((flags & getOCSPConstant(runtime, OCSP_NOCHECKS)) > 0) {
                    return runtime.getTrue();
                }

                final RubyArray chain = (RubyArray) ctx.chain(context); // never nil - we passed untrustedCerts as chain
                try {
                    if (checkIssuer(newBasicOCSPResp(), chain)) return runtime.getTrue();
                }
                catch (IOException e) {
                    throw newOCSPError(runtime, e);
                }

                if ((flags & getOCSPConstant(runtime, OCSP_NOEXPLICIT)) != 0) {
                ...
                if ((flags & getOCSPConstant(runtime, OCSP_NOCHAIN)) != 0) {
                    ...
                    if (chain.isEmpty()) return runtime.getFalse();
                    X509Cert rootCA = (X509Cert) chain.last();
                    try {
                        ...
                        LOG.debug(context.runtime, "verify (self-signed) failed", e);
                        return runtime.getFalse();
                    }
                }
            }
    ...
    private static int getOCSPConstant(final Ruby runtime, final String name) {
        return RubyFixnum.fix2int((RubyFixnum) _OCSP(runtime).getConstant(name));
    }
    ...
    private boolean checkIssuer(BasicOCSPResp basicOCSPResp, RubyArray chain) throws IOException {
        boolean ret = false;
        ...
        X509Cert signer = (X509Cert) chain.first();
        if (chain.size() > 1) {
            X509Cert signerCA = (X509Cert) chain.entry(1);
            if (matchIssuerId(signerCA, certId, singleResponses)) {
                return checkDelegated(signer);
            }
        }
        else {
    ...
    private boolean matchIssuerId(X509Cert signerCA, CertificateID certId, List<SingleResp> singleResponses) throws IOException {
        ...
        if (certId == null) {
            ...
            byte[] issuerNameDigest = md.digest(signerCA.getSubject().getX500Name().getEncoded());
            byte[] issuerKeyDigest = md.digest(SubjectPublicKeyInfo.getInstance(
                    signerCA.getAuxCert().getPublicKey().getEncoded()).getPublicKeyData().getBytes());
            if(!MessageDigest.isEqual(issuerNameDigest, certId.getIssuerNameHash())) return false;
            if(!MessageDigest.isEqual(issuerKeyDigest, certId.getIssuerKeyHash())) return false;
            return true;
        }
    }
    ...
    private java.security.cert.Certificate findSignerCert(final ThreadContext context,
        ...
        if (ret == null && (flags & RubyFixnum.fix2int((RubyFixnum)_OCSP(runtime).getConstant(OCSP_NOINTERN))) == 0) {
            ...
            for (X509CertificateHolder cert : newBasicOCSPResp().getCerts()) {
                try {
        ...
        if (respID.getName() != null) {
            ...
            for (java.security.cert.Certificate cert : certificates) {
                byte[] pubKeyDigest = Digest.digest(
                        ...
                        RubyString.newString(context.runtime,
                                SubjectPublicKeyInfo.getInstance(cert.getPublicKey().getEncoded())
                                        .getPublicKeyData().getBytes())
                        ).getBytes();
                if (MessageDigest.isEqual(respID.getKeyHash(), pubKeyDigest)) return cert;
            }
        }
        return null;
    }
    ...
    private List<X509Cert> getCertsFromResp(ThreadContext context) {
        X509CertificateHolder[] certs = newBasicOCSPResp().getCerts();
    ...
    private BasicOCSPResp newBasicOCSPResp() {
